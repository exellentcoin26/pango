...
impl<'a> Parser<'a> {
    ...
    ///
    /// [`Ast`]: super::ast::Ast
    pub(crate) fn parse(mut self) -> ParseResult<'a, ast::Ast> {
        ...
        match self.expression() {
            Ok(expr) => Ok(Ast(expr)),
            Err(errs) => Err(errs.set_source(self.source)),
        }
    }

    /// Rule: `expression ::= sub_expression (VERTICAL expression)?`
    fn expression(&mut self) -> ParseResult<'a, ast::ExprKind> {
    ...
    fn sub_expression(&mut self) -> ParseResult<'a, Vec<ast::ExprKind>> {
        if self.tokens.peek().is_none() {
    ...
    fn sub_expression_item(&mut self) -> ParseResult<'a, ast::ExprKind> {
    ...
    fn group(&mut self) -> ParseResult<'a, ast::ExprKind> {
        // LEFT_PAREN
        if !matches!(
            self.tokens.next(),
            Some(Token {
    ...
    fn match_(&mut self) -> ParseResult<'a, ast::ExprKind> {
    ...
    fn match_item(&mut self) -> ParseResult<'a, ast::LiteralKind> {
    ...
    /// RIGHT_BRACKET`
    fn character_group(&mut self) -> ParseResult<'a, ast::LiteralKind> {
        // LEFT_BRACHET
        if !matches!(
            self.tokens.next(),
            Some(Token {
    ...
    /// CHARACTER`
    fn character_group_item(&mut self) -> ParseResult<'a, ast::GroupedLiteralKind> {
