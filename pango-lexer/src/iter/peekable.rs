...
/// Peekable trait stolen from the `std::iter::Peekable` struct. This allows
/// other iterators that behave the same as peekable to implement the trait and
/// allows other traits to be automatically implemented for items that implent
/// this one. I wish the rust standard library would implement such traits.
pub trait Peekableable: Iterator {
...
impl<I: Iterator> Peekableable for Peekable<I> {
...
pub trait CachedPeekableable<I: Iterator> {
    fn cached_peekable(self) -> CachedPeekable<I>;
}

impl<I: Iterator> Iterator for CachedPeekable<I>
where
    I::Item: Clone,
{
    type Item = I::Item;
...
impl<I: Iterator> CachedPeekable<I>
where
    I::Item: Clone,
{
...
impl<I: Iterator> CachedPeekableable<I> for I {
    fn cached_peekable(self) -> CachedPeekable<I> {
        CachedPeekable {
            iter: self.peekable(),
            current: None,
...
impl<I: Iterator> Peekableable for CachedPeekable<I>
where
    I::Item: Clone,
{
    fn peek(&mut self) -> Option<&I::Item> {
    ...
    fn next_if(&mut self, func: impl FnOnce(&I::Item) -> bool) -> Option<I::Item> {
        self.iter.next_if(func).map(|next| {
            self.current
                .insert(Some(next))
                .clone()
                .expect("just inserted a Some, so this should never fail")
        })
    }
    ...
    where
        ...
        I::Item: PartialEq<T>,
    {
        self.iter.next_if_eq(expected).map(|next| {
            self.current
                .insert(Some(next))
                .clone()
                .expect("just inserted a Some, so this should never fail")
...
#[cfg(test)]
mod tests {
    use super::{CachedPeekableable, Peekableable};

    #[test]
    fn cached_peek_and_current() {
        let mut p = "He!".chars().cached_peekable();

        assert_eq!(p.peek(), Some(&'H'));
        assert_eq!(p.current(), None);
        assert_eq!(p.next(), Some('H'));
        assert_eq!(p.current(), Some('H'));
    ...
    #[test]
    fn chached_peek_and_peekable() {
        let mut p = "He!".chars().cached_peekable();

        assert_eq!(p.peek(), Some(&'H'));
        assert_eq!(p.current(), None);
        assert_eq!(p.next_if(|next| *next != 'H'), None);
        assert_eq!(p.current(), None);
        assert_eq!(p.next_if_eq(&'H'), Some('H'));
        assert_eq!(p.current(), Some('H'));

        assert_eq!(p.peek(), Some(&'e'));
        assert_eq!(p.peek(), Some(&'e'));
        assert_eq!(p.current(), Some('H'));
        assert_eq!(p.next_if(|next| *next == 'e'), Some('e'));
        assert_eq!(p.current(), Some('e'));
